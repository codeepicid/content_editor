...
class GalleryController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		if($request->hasFile('image')){
			$image = $request->url.'.'.$request->image->extension();
			$request->image->move(public_path('gallery/'),$image);

			$field2 = Gallery::find($field->id);
			$field2->location = $image;
			$field2->save();
		}
		return redirect('v/gallery')
			->with('type','success')->with('message','New image has been saved');
	}
